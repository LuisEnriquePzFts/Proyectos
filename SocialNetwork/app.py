...
app = Flask(__name__)
app.secret_key = "keysecret.database"
...
@app.route("/")
def index():
    steam = models.Post.select().limit(100)
...
def login():
    ...
    if form.validate_on_submit():
        try:
            ...
            if check_password_hash(user.pasword, form.password.data):
                login_user(user)
                flash("You've been logged im!", "success")
...
def logout():
    logout_user()
    flash("You've been logged out bro! Come back soon!","success")
    return redirect(url_for("index"))
...
def follow(username):
    try:
        ...
        try:
            ...
            flash("You are unfollowed {}".format(to_user.username),"success")
